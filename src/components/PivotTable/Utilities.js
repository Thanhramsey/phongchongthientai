...
export const addSeparators = (nStr, thousandsSep, decimalSep) => {
...
export const numberFormat = (opts_in) => {
  const defaults = {
...
function PivotData(props) {
  ...
  const aggregator = aggregators[aggregatorName](vals);
  const tree = {};
  const rowKeys = [];
  const colKeys = [];
  const rowTotals = {};
  const colTotals = {};
  const allTotal = this.aggregator(this, [], []);
  const sorted = false;
  ...
  const forEachMatchingRecord(criteria, callback) {
