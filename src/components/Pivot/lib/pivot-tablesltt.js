...
export default function PivotTable(props) {
      ...
      <tr>
        {columns.map((col) => {
          if (col.type !== "dimension") {
            return (
              <th className={classes.slttbottom} key={col.title}>
                {col.title}
              </th>
            );
          }
        ...
        {columns.map((col) => {
          if (col.type == "dimension") {
            if (col.title === dimensionst[dimensionst.length - 1]) {
              return (
                <th className={classes.slttcongthuc2} key={col.title}></th>
              );
            } else {
              return (
                <th className={classes.slttcongthuc1} key={col.title}></th>
              );
            }
          }
        })}
        <th className={classes.slttcongthuc}> (4) </th>
        <th className={classes.slttcongthuc}> (5) </th>
        <th className={classes.slttcongthuc}> (6)=(4)-(5) </th>
        <th className={classes.slttcongthuc}> (7)=(4)/(5) </th>
        <th className={classes.slttcongthuc}> (8) </th>
        <th className={classes.slttcongthuc}> (9) </th>
        <th className={classes.slttcongthuc}> (10)=(8)-(9) </th>
        <th className={classes.slttcongthuc}> (11)=(8)/(9) </th>
        <th className={classes.slttcongthuc}> (12) </th>
        <th className={classes.slttcongthuc}> (13) </th>
        <th className={classes.slttcongthuc}> (14)= (12)-(13) </th>
        <th className={classes.slttcongthuc}> (15)=(13)/(13) </th>
        <th className={classes.slttcongthuc}> (16) </th>
        <th className={classes.slttcongthuc}> (17) </th>
        <th className={classes.slttcongthuc}> (18)=(16)-(17) </th>
        <th className={classes.slttcongthuc}> (19) </th>
        <th className={classes.slttcongthuc}> (20)=(18)/(19) </th>
        <th className={classes.slttcongthuc}> (21) </th>
        <th className={classes.slttcongthuc}> (22)=(16)/(21) </th>
      </tr>
    );
  };
  ...
  const sumvalue = (value, rows) => {
    ...
    if (value === "tL_HTKH_NAMTotal") {
      kq = (
        ((Number(sumArray[12]) - Number(sumArray[13])) / Number(sumArray[15])) *
        100
      ).toFixed(2);
      kq = Number(kq).toLocaleString();
    }
    if (value === "psC_TLCKTotal") {
      kq = ((Number(sumArray[12]) / Number(sumArray[17])) * 100).toFixed(2);
      kq = Number(kq).toLocaleString();
    }
    if (kq === "NaN") kq = "";
    return (
