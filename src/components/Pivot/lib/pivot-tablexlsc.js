...
export default function PivotTable(props) {
      ...
      <tr>
        {columns.map((col) => {
          if (col.type !== "dimension") {
            return (
              <th className={classes.xlscbottom} key={col.title}>
                {col.title}
              </th>
            );
          }
  ...
  const congthuc = () => {
    return (
      <tr>
        {columns.map((col) => {
          if (col.type == "dimension") {
            return <th className={classes.xlsccongthuc1} key={col.title}></th>;
          }
        })}
        <th className={classes.xlsccongthuc}> (4) </th>
        <th className={classes.xlsccongthuc}> (5) </th>
        <th className={classes.xlsccongthuc}> (6) </th>
        <th className={classes.xlsccongthuc}> (7) </th>
        <th className={classes.xlsccongthuc}> (8)=(7)/(5) </th>
        <th className={classes.xlsccongthuc}> (9) </th>
        <th className={classes.xlsccongthuc}> (10) </th>
        <th className={classes.xlsccongthuc}> (11) </th>
        <th className={classes.xlsccongthuc}> (12) </th>
        <th className={classes.xlsccongthuc}> (13)=(12)/(10) </th>
      </tr>
    );
  };
  ...
  const sumvalue = (value, rows) => {
    ...
    if (value === "tL_DUNGTG_NAMTotal") {
      kq = ((Number(sumArray[8]) / Number(sumArray[6])) * 100).toFixed(2);
      kq = Number(kq).toLocaleString();
    }
    if (kq === "NaN") kq = "";
    return (
