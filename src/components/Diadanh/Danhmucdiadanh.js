...
const Diadanh = React.memo(function Diadanh(props) {
  const classes = useStyles();
  const { diadanhrows, diadanhcolumns } = props;
      ...
      <GridItem xs={12} sm={10} md={12}>
        <div style={{ height: 500, width: "100%" }}>
          <DataGrid
            getRowId={(row) => row.id}
            rows={diadanhrows}
            columns={diadanhcolumns}
            rowsPerPageOptions={[]}
            hideFooter={true}
            checkboxSelection={false}
            onSelectionModelChange={(ids) => {
              const selectedIDs = new Set(ids);
              const selectedRows = diadanhrows.filter((row) =>
...
Diadanh.propTypes = {
  diadanhrows: PropTypes.array,
  diadanhcolumns: PropTypes.array,
};
export default Diadanh;
